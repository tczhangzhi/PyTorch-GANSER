...
# CUDA_VISIBLE_DEVICES=3 python finetune_proposed_woramask_pretrain.py
RECEIVED_PARAMS = {
    "c_lr": 0.00001,
    "weight_ce": 0.5
}

TRAIL_ID = 'cross_validation_finetune_woramask' + str(CFG.FOLD)
...
class Trainer():
    ...
    def load(self):
        ...
        if os.path.exists('./parameters/cross_validation_backbone' +
                          str(CFG.FOLD) + '.pth'):
            c_model_state_dict = torch.load(
                './parameters/cross_validation_backbone' + str(CFG.FOLD) +
                '.pth')
