...
# CUDA_VISIBLE_DEVICES=3 python finetune_proposed_woramask_pretrain_arousal.py
RECEIVED_PARAMS = {
    "c_lr": 0.00001,
    "weight_ce": 0.5
}

TRAIL_ID = 'cross_validation_finetune_woramask_arousal' + str(CFG.FOLD)
...
class Trainer():
    ...
    def training_step_c_model(self, batch, batch_idx):
        ...
        aug_x = self.g_model(x).detach()
        aug_y_hat, aug_x_feat = self.c_model(aug_x)

        loss += RECEIVED_PARAMS['weight_ce'] * self._loss_fn_ce(aug_y_hat, y)
    ...
    def load(self):
        gan_model_state_dict = torch.load(
            './parameters/cross_validation_proposed_woramask_pretrain.pth')
        self.g_model.load_state_dict(gan_model_state_dict['g_model'])

        if os.path.exists('./parameters/cross_validation_backbone_arousal' +
                          str(CFG.FOLD) + '.pth'):
            c_model_state_dict = torch.load(
                './parameters/cross_validation_backbone_arousal' + str(CFG.FOLD) +
                '.pth')
