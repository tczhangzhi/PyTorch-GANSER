...
from preprocessors import FourClassificationLabel
...
# CUDA_VISIBLE_DEVICES=3 python finetune_proposed_pretrain_fuse.py
RECEIVED_PARAMS = {
    "c_lr": 0.00001,
    "weight_ssl": 0.5,
}

TRAIL_ID = 'cross_validation_finetune_fuse' + str(CFG.FOLD)
...
preprocessors_results = DEAPDataset(
    ...
    feature_preprocessors)('./dataset/deap_four_dataset.pkl')
...
class Trainer():
    ...
    def load(self):
        ...
        if os.path.exists('./parameters/cross_validation_backbone_fuse' +
                          str(CFG.FOLD) + '.pth'):
            c_model_state_dict = torch.load(
                './parameters/cross_validation_backbone_fuse' + str(CFG.FOLD) +
                '.pth')
