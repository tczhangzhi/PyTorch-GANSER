...
TRAIL_ID = 'cross_validation_finetune_dreamer_arousal' + str(CFG.FOLD)
...
class Trainer():
    ...
    def load(self):
        ...
        if os.path.exists('./parameters/cross_validation_backbone_dreamer_arousal' +
                          str(CFG.FOLD) + '.pth'):
            c_model_state_dict = torch.load(
                './parameters/cross_validation_backbone_dreamer_arousal' + str(CFG.FOLD) +
                '.pth')
